...
function encrypt_function {
    ...
    case $method_choice in
        1)  # xxd
            if [[ -n "$output_file" ]]; then
                sudo xxd -p "$input_file" > "$output_file"
            else
                sudo xxd -p "$input_file"
            fi
            ;;
        2)  # base64
            if [[ -n "$output_file" ]]; then
                sudo base64 "$input_file" > "$output_file"
            else
                sudo base64 "$input_file"
            fi
            ;;
        3)  # openssl
            if [[ -n "$output_file" ]]; then
                sudo openssl enc -aes-256-cbc -in "$input_file" -out "$output_file"
            else
                echo "Output file is required for openssl."
            fi
            ;;
        4)  # 7-Zip
            sudo 7z a -mhe=on archive.7z -p "$input_file"
            ;;
        5)  # steghide
            read -p "Enter the cover file name: " cover_file
            if [[ -n "$output_file" ]]; then
                sudo steghide embed -cf "$cover_file" -ef "$input_file" -sf "$output_file"
            else
                sudo steghide embed -cf "$cover_file" -ef "$input_file"
            fi
            ;;
        *)
...
function WPA_Supplicant_function {
    echo "choose WPA option:"
    echo "1) Create .conf"
    echo "2) Run .conf"
    read -p "Enter your choice (1 or 2): " choice
    ...
    case $choice in 
         1) 
            ...
            read -p "Enter file.conf name: " file
            wpa_passphrase "$router" "$passphrase" > "$file"
            echo "Configuration file $file created successfully."
            ;;
         2)
            read -p "Enter file.conf name: " file 
            sudo wpa_supplicant -B -i wlan0 -c "$file"
            echo "wpa_supplicant started with $file."
            ;;
         *)
...
while true; do
    ...
    case $choice in
        ...
        6) encrypt_function ;;
        7) ARP_scan_function ;;
        8) NMAP_scan_function ;;
        9) WPA_Supplicant_function;;
